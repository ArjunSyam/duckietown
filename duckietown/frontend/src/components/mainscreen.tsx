...
import { SearchBar } from "./searchbar";
import { AIPanel } from "./aipanel";
import type { FileRecord, FileTypeFilter, ViewMode, SearchResult } from "../types";
...
export function MainScreen({
  ...
  const [aiPanelOpen, setAiPanelOpen] = useState(false);
  const [aiTargetFile, setAiTargetFile] = useState<string | null>(null);
  // Increment to force AIPanel to remount with fresh state on each open
  ...
  const openAiPanel = (targetFile: string | null) => {
    ...
    setAiPanelOpen(true);
    // Always increment so AIPanel remounts fresh — no stale context ever
        ...
        <div className="flex flex-col flex-1 overflow-hidden min-w-0">
          ...
          {aiResults.length > 0 && (
            <div className="flex items-center gap-2 px-4 py-1.5 bg-indigo-500/10 border-b border-indigo-500/20 shrink-0">
              <Sparkles size={11} className="text-indigo-400" />
              <span className="text-[11px] text-indigo-300">
                {displayFiles.length} files ranked by relevance for &ldquo;{searchQuery}&rdquo;
              </span>
              <button
                onClick={() => { setAiResults([]); setSearchQuery(""); }}
                className="ml-auto text-[10px] text-indigo-400/60 hover:text-indigo-400 transition-colors"
              >
                Clear
              </button>
          ...
          <SearchBar
            ...
            onToggle={() => setSearchOpen((v) => !v)}
            query={searchQuery}
            onSearch={(q) => { setSearchQuery(q); if (!q) setAiResults([]); }}
            onAiSearch={setAiResults}
            aiResultCount={aiResults.length}
          />
        </div>

        {/* AI Panel — fixed width, never causes layout shift on file area */}
        {aiPanelOpen && (
          <AIPanel
            key={aiSessionKey}
            open={aiPanelOpen}
