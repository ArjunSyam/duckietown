...
export default function App() {
  ...
  useEffect(() => {
    wails.on("auth-success", (data: unknown) => {
      ...
      setUser(u);
      toast.success("Signed in!", { description: u.email });
      wails.getVaultPath().then((path) => {
        if (path) {
          setVaultPath(path);
          loadFiles();
    ...
    wails.on("auth-restored", (data: unknown) => {
      const u = data as User;
      setUser(u);
    });

    wails.on("auth-expired", () => {
      setUser(null);
      setFiles([]);
      setVaultPath(null);
      setIsWatching(false);
    ...
    wails.on("watcher-ready", () => {
      setIsWatching(true);
      toast.success("Duckietown active", { description: "Watching for changes" });
    });

    return () => {
      wails.off("auth-success");
      wails.off("auth-error");
      wails.off("auth-restored");
      wails.off("auth-expired");
      wails.off("file-uploaded");
      wails.off("file-changed");
      wails.off("upload-error");
      wails.off("watcher-ready");
  ...
  const handleSignOut = async () => {
    try {
      ...
      // signOut may throw if session is already gone — still clear UI
    }
    // Clear all state before switching screen
    setUser(null);
    setFiles([]);
    setVaultPath(null);
    setIsWatching(false);
    ...
    toast.dismiss("sync");
    toast.info("Signed out");
    // Small delay ensures Go backend has cleared session before auth screen mounts
    setTimeout(() => setScreen("auth"), 100);
  };
  ...
  const handleGetPreview = useCallback(async (name: string): Promise<string> => {
    try {
      return await wails.getFilePreview(name);
    } catch {
      return "";
