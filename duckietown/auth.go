...
var supaHTTPClient = func() *http.Client {
	...
	if ok {
		...
		// Force HTTP/1.1. We do this by disabling HTTP/2 negotiation.
		// This avoids rare "http2: response body closed" issues observed when decoding
		// Supabase responses.
...
<body>
  ...
  <script>
    ...
    if (accessToken) {
      ...
      fetch('/token', {
        ...
        if (res.ok) {
          ...
          document.getElementById('title').textContent = 'Something went wrong';
          document.getElementById('msg').textContent   = errorText || 'Please try again.';
        }
      }).catch(err => {
        console.error('Network error:', err);
        document.getElementById('title').textContent = 'Network error';
        document.getElementById('msg').textContent   = err.message || 'Please check your connection.';
      });
    } else {
	...
	mux.HandleFunc("/token", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Printf("Received token request body: %s\n", string(body))
		...
		if err := json.Unmarshal(body, &tokens); err != nil {
			fmt.Printf("Error unmarshaling tokens: %v\n", err)
			w.WriteHeader(http.StatusBadRequest)
			return
		}

		fmt.Printf("Extracted tokens - Access: %s..., Refresh: %s...\n",
			func() string {
				if len(tokens.AccessToken) > 20 {
					return tokens.AccessToken[:20]
				}
				return tokens.AccessToken
			}(),
			func() string {
				if len(tokens.RefreshToken) > 20 {
					return tokens.RefreshToken[:20]
				}
				return tokens.RefreshToken
			}())

		if tokens.AccessToken == "" {
			fmt.Printf("No access token received\n")
			wailsruntime.EventsEmit(a.ctx, "auth-error", "no access token received")
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		...
		// Fetch user info from Supabase
		fmt.Printf("Fetching user info from Supabase...\n")
		user, err := a.supaGetUser(tokens.AccessToken)
		if err != nil {
		...
		fmt.Printf("Successfully authenticated user: %s (%s)\n", user.Email, user.ID)
...
func (a *App) supaGetUser(accessToken string) (AuthUser, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return AuthUser{}, fmt.Errorf("supabase /auth/v1/user failed: %d: %s", resp.StatusCode, strings.TrimSpace(string(b)))
	}
